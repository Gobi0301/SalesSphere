...
<script>
	$(document).ready(function() {
		...
		$('#submit').click(function(e) {
			...
			if (firstName == '') {
				isValid = false;
				$("#salesOrderNoErr").show();
				$("#salesOrderNoErr").html("Please enter SalesorderNO");
				$("#firstName").css({
	...
	<div class="item">
		<div class="row ">
			<h3 class="text-center no-margin titleunderline underline"
				style="margin-top: -10px;">List SalesOrder</h3>
			<sec:authorize
			...
			<form:form id="myForm" name="pageNo" method="post"
				class="login-form clearfix" action="search"
					...
					<p>
						<a class="btn btn-theme btn-xs btn-default"
							style="color: #1b1818; font-weight: bold;"><c:out
								value="${viewCount}"></c:out></a> <strong class="color-black">Salesorder</strong>
					</p>
				</div>
			...
			<c:if test="${not empty count}">
				...
				<div class="col-sm-12" style="margin-top: 20px">
					<p>
						<a class="btn btn-theme btn-xs btn-default"
							style="color: #1b1818; font-weight: bold;"><c:out
								value="${count}"></c:out></a> <strong class="color-black">Salesorder</strong>
					</p>
				</div>
					...
					<div class="pi-responsive-table-sm">
						<div class="pi-section-w pi-section-white piTooltips">
							<display:table id="data" name="${listSales.list}"
								requestURI="/view-sales-order" export="false"
								...
								<display:column property="accountBO.accountName"
									...
									<sec:authorize
									access="hasAnyRole('ROLE_SALES_MANAGER','ROLE_COMPANY')">
								...
								<sec:authorize
									access="hasAnyRole('ROLE_SALES_MANAGER','ROLE_COMPANY','ROLE_ADMIN')">
									<display:column url="create-invoice" media="html" paramId="id"
										paramProperty="id" title="View Invoice">
										<a href="createinvoice?salesno=${data.salesOrderId}"> View
											Invoice </a>
									</display:column>
								</sec:authorize>
				...
				<nav style="text-align: center;">
					<ul class="pagination pagination-theme  no-margin center"
						...
						<c:if test="${listSales.currentPage gt 1}">
							<li><a href="view-sales-order?page=1&&searchElement=${searchElement}"><span><i
										class="fa fa-angle-double-left" aria-hidden="true"></i> </span></a></li>
							<li><a
								href="view-sales-order?page=${listSales.currentPage - 1}&&searchElement=${searchElement}"><span><i
						...
						<c:forEach items="${listSales.noOfPages}" var="i">
							<c:choose>
								...
								<c:otherwise>
									<li><a href="view-sales-order?page=${i}&&searchElement=${searchElement}">${i}</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<c:if test="${listSales.currentPage lt listSales.totalPages }">
							...
							<li><a
								href="view-sales-order?page=${listSales.lastRecordValue}&&searchElement=${searchElement}"><span><i
					...
					<div class="pi-responsive-table-sm">
						<div class="pi-section-w pi-section-white piTooltips">
							<display:table id="data" name="${listSalesSearch.list}"
								...
								<display:column property="date" title="Date" />
								<sec:authorize
									access="hasAnyRole('ROLE_ADMIN','ROLE_SALES_MANAGER','ROLE_COMPANY','ROLE_CREATE_INVOICE')">
									<display:column url="create-invoice" media="html" paramId="id"
										paramProperty="id" title="View Invoice">
										<a href="createinvoice?salesno=${data.salesOrderNo}"> View
											Invoice </a>
									</display:column>
								</sec:authorize>
					...
					<ul class="pagination pagination-theme  no-margin center"
						...
						<c:if test="${listSalesSearch.currentPage gt 1}">
							<li><a href="view-sales-order?page=1&&searchElement=${searchElement}"><span><i
										class="fa fa-angle-double-left" aria-hidden="true"></i> </span></a></li>
							<li><a
								href="view-sales-order?page=${listSalesSearch.currentPage - 1}&&searchElement=${searchElement}"><span><i
										class="fa fa-angle-left" aria-hidden="true"></i> </span></a></li>
						</c:if>
						...
						<c:if
							test="${listSalesSearch.currentPage lt listSalesSearch.totalPages }">
							<li><a
								href="view-sales-order?page=${listSalesSearch.currentPage + 1}&&searchElement=${searchElement}"><span><i
										class="fa fa-angle-right" aria-hidden="true"></i> </span></a></li>
							<li><a
								href="view-sales-order?page=${listSalesSearch.lastRecordValue}&&searchElement=${searchElement}"><span><i
										class="fa fa-angle-double-right" aria-hidden="true"></i> </span></a></li>
						</c:if>
